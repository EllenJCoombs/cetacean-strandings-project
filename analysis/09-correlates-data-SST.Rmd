...
## Load packages 
```{r}
library(chron) #Provides chronological order for dates and times 
library(RColorBrewer) #If you want to plot SST maps 
...
Once this code has been run for each of the 14 sites, a dataframe of SST should be available for each of the sites, e.g. Ireland Wicklow. If desired, these should be saved as csv files, for example write.csv(Ireland_WIcklow, file = "Ireland_Wicklow.csv") 

Lat/longs and locations required: 
<Ellen to add>
...
Pulling out the monthly mean from thw 14 locations and then selecting the mean yearly max SST
```{r}
UK_Ireland_SST <- read.csv("UK_Ireland_SST.csv")

#Added a yearly mean for each location in Excel - Think, can't find the code for this 
