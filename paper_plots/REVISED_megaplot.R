...
plines <- plines + annotate("rect", xmin=1914, xmax=1918, ymin=0, ymax=800, alpha=.1, fill="gray22") +
  ...
  annotate("segment", x =1985, xend=1985, y=50, yend=750, colour = "gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) +
  geom_text(
    aes(x = 1988, y = 770, label = "1985/1986 season:
        Moratorium on whaling comes into effect"), size = 3, colour = "gray38") +
  #Annotation coordinates 
  annotate("segment", x = 1990, xend = 1990, y = 50, yend = 585, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
    aes(x = 1992, y = 610, label = "1990: CSIP and IWDG
        programmes start"), size = 3, colour = "gray38") +
  annotate("segment", x = 1947, xend = 1947, y = 50, yend = 550, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
    aes(x = 1953, y = 430, label = "1950s: Increase in 
        post-war fishing & 
        whaling effort"), size = 3, colour = "gray38") + 
  annotate("segment", x = 1953, xend = 1953, y = 150, yend = 400, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
  ...
  annotate("segment", x = 1920, xend = 1920, y = 50, yend = 400, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
    aes(x = 1920, y = 420, label = "1920s: Sonar is used 
        in UK and French waters"), size = 3, colour = "gray38") +
  annotate("segment", x = 2000, xend = 2000, y = 50, yend = 110, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
    aes(x = 2000, y = 140, label = "2000s: Increase in 
        pile-driving for offshore 
        wind turbines"), size = 3, colour = "gray38") +
  annotate("segment", x = 1960, xend = 1960, y = 50, yend = 90, colour="gray38", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
