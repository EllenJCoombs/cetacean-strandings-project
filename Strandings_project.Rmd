...
Can be done for any chosen species - this is an example here 
...
beakers <- c("Hyperoodon ampullatus", "Mesoplodon densirostris", "Mesoplodon europaeus", "Mesoplodon mirus", "Ziphius cavirostris")
...
###Species decadal hex maps
I will use this to show spatial and temporal change for all and each species 
```{r, include=FALSE}
...
Playing around with species richness plots 
...
##B. Link to sea surface temperature (SST)
Still working on this - looking at HadISST1 data (UK SST 1870-present monthly recordings)
Seperate Rmd - [Strandings_project.Rmd](Strandings_project.Rmd)
...
I used this code to look at military testing and beaker strandings i.e. split into military testing months/periods to see if there was a link between activity and strandings 
...
Plotting total stranded whales of the species that were/are hunted 
```{r, include=FALSE}
...
###Adding some key events to the whaling/catch data plot 
Ugly plot but this is the general idea! 
```{r, include=FALSE}
...
Each seperate species stranding and whaling on the same plot - split by facet_wrap species
...
Here, I am looking at whether there is a relationship between hunted and stranded species - just looking at the regression 
```{r, include=FALSE}
...
- OBIS-Seamap is static, have been told to use JCP data but this is also static - have looked at both: see "Strandings analysis" Excel doc  
...
extra_counties <- read.csv("cleaned-data/Ireland_Scot_Wales.csv")
...
#Bring in the data 
pop_vs_strandings <- read.csv("cleaned-data/Population_vs_stranding.csv")
...
yearoverview <- speciesyearcount %>% left_join(totalcount, by = "Year") %>%
  arrange(Year) %>%
  dplyr::rename(species_stranding = n.x) %>%
  dplyr::rename(all_strandings = n.y) %>%
  mutate(sampling = species_stranding/all_strandings)
...
#Other plots include: 
- Focusing on data from CSIP only: 1990 onwards 
- Military zone plots and beakers (supplementary)
- Heatmap of shipstrike incidents and physical trauma (supplementary)
...
- "Strandings analysis"
- Want to look at one more range source e.g. Carwadine 
...
2. Remove manky specimens and unknowns - sampling bias 
