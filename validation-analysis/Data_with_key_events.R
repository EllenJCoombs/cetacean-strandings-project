...
#WWI
p <- p + annotate("rect", xmin=1914, xmax=1918, ymin=0, ymax=60, alpha=.1, fill="gray44") +
geom_text(
  aes(x = 1916, y = 60, label = "WWI"), size = 3)

#WWII
p <- p + annotate("rect", xmin=1939, xmax=1945, ymin=0, ymax=60, alpha=.1, fill="gray44") +
  geom_text(
    aes(x = 1942, y = 60, label = "WWII"), size = 3)

#Moratorium 1986
p <- p + annotate("rect", xmin=1985, xmax=1986, ymin=0, ymax=60, alpha=.3, fill="gray44") +
  geom_text(
    aes(x = 1985, y = 60, label = "Moratorium comes into effect:
1985/1986 season start"), size = 3)

#Catches under objection 
p <- p + annotate("rect", xmin=1986, xmax=2015, ymin=0, ymax=60, alpha=.1, fill="gray44") +
  geom_text(
...
p <- p + annotate("segment", x = 1990, xend = 1990, y = - 0, yend = -10, colour="black", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
    aes(x = 1990, y = -12, label = "CSIP programme starts"), size = 3)

p <- p + annotate("segment", x = 1945, xend = 1945, y = 0, yend = -10, colour="black", size=0.5, arrow=arrow(length=unit(0.1,"cm"))) + 
  geom_text(
